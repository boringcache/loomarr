...
Generated recursively from each package's own doc comment and exact internal imports, using full paths below `internal/`, so it cannot drift from the code the way a hand-maintained list does. **Layer** is derived: the longest path from that package to one with no internal dependencies. It is the measured layering, not an aspirational one.
...
Store interface:
  ...
  # scheduling
  GetChannel/SaveChannel/ListChannels
  ClaimDueChannels/AttachTunarrChannel/SetChannelBroadcastCodec
...
- **Proposals:** `denied` purged after `PROPOSALS_RETENTION` (default 90d). ⚠ **`approved` and `submitted` are kept indefinitely**, for different reasons: an approved proposal is the audit trail behind `approved_by` (the record of a decision that spent real resources), and a `submitted` one is a member still waiting for an answer — ageing it out would silently discard a request rather than decline it.
  - **A terminal decision is a compare-and-swap from `submitted`.** Approval first derives the
    complete local result, then writes the final audited proposal, every newly tracked
    `available`/`wanted` title, and its intent-bound `building` channel in one database
    transaction; denial uses the same expected-state guard. A concurrent approve, deny, or
    retry therefore has exactly one winner. A failed approval leaves the proposal, title
    catalog, and channel catalog unchanged — there is no valid state in which acquisitions or
    a channel exist because of a proposal that still says `submitted`, nor an `approved`
    proposal whose local channel is missing. The non-empty channel `intent_ref` is unique at
    the database boundary, so concurrent materialization cannot mint two channels for one
    intent. Channel-number and intent uniqueness races roll the whole attempt back and make
    the coordinator reload + replan before retrying; they never leak a half-decision to the
    caller. Codec derivation and the immediate Tunarr reconcile run only after commit and are
    best-effort: the committed channel is already `building` with a due reconcile deadline, so
    the periodic channel sweep is the durable external retry. A denial can never overwrite an
    approval that already spent resources.
...
- **Every channel-row mutation advances one monotonic `revision`.** Full channel saves are
  compare-and-swap writes against the revision they read; claims and targeted updates advance the
  same counter. Approval and reconcile may reload/replan after a stale write, but an operator's
  PATCH carries the revision shown in the editor and receives 409 when stale — silently replaying
...
Flavor via `LIBRARY_FLAVOR`. A series counts as in-library when the show exists; season-level
...
| POST | `/v1/system/llm/test` | Validate a hosted provider + key **without** swapping (admin, §8.1) — the "test my key" check. Returns reachable/authorized + an error hint on failure. The UI keeps that authorization result distinct from lineup readiness: a valid key with no selected tool-capable lineup model is reported as authorized but incomplete, and full AI health remains false until both are present. |
| POST | `/v1/system/llm/pull` | Start an Ollama `pull` of a model (admin, §8.1; **local-only**, 409 on a hosted provider). Accepts **any** tag — a catalog model, a browse-to-download `pullRef` (`hf.co/…`), or a hand-typed name. Returns a `jobId` (retained as the wire name for compatibility) identifying the durable interactive-operation read below; `/v1/events` accelerates progress display. Idempotent. |
...
| GET | `/v1/events` | SSE stream of state changes. Frame `event:` types: `title` (provisioning) · `channel` (lineup/health) · `suggestion` (generation progress — `searching`→`reasoning`→`scoring`→`done`/`failed`, payload `{jobId, phase}`) · `llm_pull` (model-download percent; recover via its pull-operation GET) · `filler_ingest` (clip-download progress; recover from the acquisition run) · `filler_split` (compilation-detection progress; recover via its split-operation GET, then read a successful proposal) · `activity` (a new Dashboard feed row was written — §12, V32) · `health` (Current Health changed; empty invalidation payload — §17). Latency-only: a dropped frame is never a correctness bug — every accepted asynchronous operation has an authoritative `GET` resource on reconnect (§8). An SSE-only terminal result is not a valid operation contract. |
...
- **System probe** (`GET /v1/system/llm`, admin) returns the active provider + model and **two curated catalogs**:
  ...
  - **Hosted** — the probe returns **OpenRouter** (curated in-code: label, base URL, keys-URL, note — stable) and a **Custom** template (the user fills base URL + key). When the operator switches to the hosted path with no URL already saved or staged, the form seeds OpenRouter's returned `baseUrl`; it never asks them to reproduce a value the backend already owns. A non-empty value is preserved, so Custom remains a first-class editable endpoint. For either, the **model list is live** (`{baseURL}/models`), and recommendations rank it for **Loomarr's use case — best grounded tool-caller, not merely cheapest**. Ranking, over the live list:
    1. **Hard filter (capability + request mode):** keep only models the provider advertises as tool-calling (`supported_parameters` has `tools`) — grounding is impossible without it — and exclude OpenRouter's `:batch` variants. Loomarr's lineup path uses synchronous chat completions, so a model available only through the Batch API is not a selectable model even when the catalog advertises tools. The same `:batch` guard applies to direct OpenRouter selection requests, not just the displayed catalog.
    ...
    The first high-tier model after ranking is the **single safe default**. Other tiered, live, tool-capable models retain plain-English rationales but are alternatives, not additional recommendations. The UI leads with that default and one primary action, follows it with at most two strong alternatives, and keeps the complete provider catalog behind **Show all models** progressive disclosure. If the active model is outside the shortlist it remains visible, so disclosure never obscures the current state. A tool-capable model **not in any tier still appears** (selectable), just **unranked** — we never hide a new model we haven't tiered yet. A **thin** provider whose `/models` returns bare ids degrades to the live id list, unranked and **not selectable until tool support is known**: Loomarr's grounding loop requires tool-calling, so absence of capability evidence cannot be presented as a safe choice. Each provider carries a **`keyConfigured`** flag; the API key is **never echoed back**. The one per-model hardcode is a tiny pre-key **fallback**, shown before credentials are configured with its own `recommended`, `tools`, and plain-English `why`; it is a preview of the safe default, and becomes selectable after the key is configured. The normal Settings page does not expose a second free-text model field beside the picker.
...
- **Select** (`POST /v1/system/llm/select`, admin) sets the active provider **and** model. For a **local** model it verifies the tag is pulled (409 if not — pull it first). For **OpenRouter** it accepts an optional `apiKey` and requires OpenRouter's namespaced `vendor/model` slug; an Ollama-style `name:tag` (or any other unnamespaced value) and a batch-only `:batch` variant are rejected before a provider call. For **Custom** it accepts a `baseUrl` (any OpenAI-compatible `/v1`) plus its `apiKey`; Custom model ids remain provider-defined because this path deliberately covers local OpenAI-compatible runtimes too. Before committing it **validates** the endpoint + key with a cheap live call (a tiny models-list / completion) and rejects a bad base or key (**401/502**) rather than letting the next suggestion job fail opaquely — so only a reachable, authorized endpoint is ever committed (this validation, not a hardcoded allowlist, is what gates a user-supplied base URL). It then persists to the **settings store** (§5: `llm.provider`, `llm.model`, `llm.url`, and the **secret** `llm.api_key`) and **hot-swaps** the live suggester via an atomic pointer — effective on the next job, **no restart**. These persisted settings **override** their §15 env defaults when present, so an in-app choice survives reboots. The stored key rides in the settings table (plaintext, same trust boundary as the `LLM_API_KEY` env var it replaces — a household LAN app; §11) and is **never returned** by any GET.
- **Test** (`POST /v1/system/llm/test`, admin) validates a provider+key **without** committing a swap — the "test my key" button. Returns reachable/authorized + the error on failure. Uses the same cheap validation call as select. Settings and the wizard expose this as an explicit **Test provider credentials** action, separate from **Check AI readiness**: a rejected key fails regardless of model selection; an accepted key with no model says **authorized** and names model selection as the remaining step; only an authorized provider plus a selected tool-capable lineup model is fully ready. The standing health poll does not run this inference-spending credential test.
...
- **Pull** (`POST /v1/system/llm/pull`, admin) is **local-only**: it starts one generation-owned, explicitly timed interactive operation. It accepts **any** tag — a bare `pullRef` from discover (`hf.co/<repo>`), or a plain tag — Ollama errors cleanly on an unresolvable one. The returned `jobId` reads durably at `GET /v1/system/llm/pull-operations/{jobId}`; `/v1/events` streams the same progress only to reduce visible latency. Startup terminates an unfinished prior-generation row as interrupted rather than pretending to resume Ollama's partial stream. Idempotent. A hosted provider has nothing to download (409 if called on one).
...
All links use a configured recipient-reachable `access.public_url`; the backend never infers an async
or QR destination from `Host` or forwarded headers. When the setting is empty, the General editor may
stage the browser's current origin for the administrator to confirm and save. The setting is distinct
...
### Stack & delivery
- **Current delivery:** React 19 + TypeScript (Vite SPA), TanStack Router, TanStack Query, and
  Tailwind/shadcn remain the shipping Web stack only for surfaces not yet migrated under #970.
- **Target client platform:** Loomarr-owned modules implemented with Tamagui Core render through the
  existing Vite web adapter and Expo/React Native adapters for iOS, Android, Android TV, and Apple
  TV. The 2026-09-03 maintainer decision in #970 authorizes complete Shield and Web parity migration:
  each client preserves its own current approved presentation and behavior, with no cross-client
  convergence or refinement until the Kotlin and legacy Web implementations are retired.
...
- **Guide** (route `/guide`) — headed **"Channels"**, this is the single channels surface: a cross-channel time grid answering both *"what do I have"* and *"what is on"*. **Origination** (how a channel is born) is a header action on this surface: the everyday door is **"✦ Add a channel" → describe it** (the §13 describe→review→approve flow, inlined below the header), and an empty install shows the **"Dead air"** state whose one action opens the same panel. **Evolution** (shaping a live channel) happens on the detail page (`/channels/{id}`) and never re-originates it.
  ...
  **Channel surface map** (the guardrail against capability-without-UI — a PR adding a channel capability updates this table):
  ...
  | subtitles | none — track discovery only; burn-in control retired in V55 until the encoder implements it | none | — |
  ...
  | ordering; separation (no-repeat, series gap, block cap) | `policy.ordering`, `policy.separation.*` | Programming → How it's ordered. **One play-order control, not separate Playback + Ordering controls:** an unset policy choice is labelled `Use channel strategy (<resolved strategy>)`, naming the stored fallback without presenting `Channel.Strategy` as a second editable knob. Unset separation fields say they use Loomarr's built-in spacing; they never claim blank means “no restriction,” because `Resolved` fills those fields from built-in constants. | admin |
  ...
  | seasonal mode / holidays / off-season | `policy.seasonal.*` | Programming → When it changes (below the curation rules — the calendar is the longest of the three clocks on that surface). The default is named as Loomarr's **built-in seasonal rotation**, not the source-free “Automatic”; wire values `""` and `"auto"` render as one choice because they resolve identically. The holiday list is read from the **rule vocabulary** (`when` tokens `holiday:<id>`), not hand-mirrored, so it cannot offer a holiday `builtinCalendar` does not know. An empty stored holiday subset means all built-ins, so the UI shows every holiday checked; unchecking one materializes the remaining subset. Off-season fallback renders **only** in `exclusive` mode, the only mode that consults it, and its identical `""` / `"loop"` states render as one choice. | admin |
...
**Companion design docs** (authoritative for their own domains): `programming-design.md` (ChannelPolicy heuristics — §8/§9), `config-design.md` (settings registry mechanics — §13/§15), and `frontend-design.md` (the shared client platform and its legacy-to-Tamagui migration — §12/§14).
...
ccache's HTTP storage helper is pinned to 0.9. Both providers use Expo's documented
`pch_defines,time_macros` setting for precompiled headers. Worklets receives
Clang's `-fno-pch-timestamp` through appended compiler flags, matching the existing
Expo modules and Reanimated setup without replacing compiler defaults.
...
| Release-note classification | **OpenRouter structured output via plain `net/http`**, defaulting to `openai/gpt-5-mini`; GitHub remains the source of PR titles, authors, links, contributors, and compare ranges | Release notes get useful, Uptime-Kuma-style sections for pennies per release without another application runtime or SDK. The model receives one closed object whose required property names are the exact real PR numbers and whose values are one of the seven category identifiers; this encodes one assignment per PR without repeating the entire id set across category arrays. Deterministic Go still rejects missing, invented, extra, duplicate-key, or malformed output and renders only GitHub-authored bullets. Publication fails closed when inference is unavailable. |
...
### Client platform (Node 22.5+, pnpm, React 19 + TypeScript)
| Concern | Decision | Why |
| --- | --- | --- |
| Workspace and task graph | **pnpm workspaces + Turborepo**, subordinate to the repository Make targets | pnpm remains the dependency and workspace authority. Turborepo adds content-aware task dependencies, caching, and affected execution inside `web/`; it does not replace Make, GitHub's impact classifier, generated-artifact checks, or release gates. The first scaffold must prove that unrelated Go-only work does not build native clients and that a shared-package edit invalidates every real consumer. |
...
| Package dependency seams | **`dependency-cruiser` 18** + analyzer-only **TypeScript 6** (pnpm package extension) | Each `web/packages/<name>/` package is a deep module: root TypeScript files are the small public interface and nested folders are private implementation. Focused `@loomarr/api/{models,endpoints,zod}/*` and `@loomarr/core/*` exports still preserve the route-payload boundary in frontend-design §4.4; they resolve to focused root entry files rather than exposing generated or `src/` paths. Orval regenerates the API entry files with its client. A static import-graph gate prevents app code and sibling packages from bypassing those entry points and rejects cycles; this turns package locality into an enforced property instead of a review convention. Version 18 preserves Loomarr's Node 22 floor but its parser rejects TypeScript 7, so pnpm supplies TypeScript 6 only beside dependency-cruiser; each workspace package continues compiling with its own TypeScript 7. Neither tool ships in browser code. |
...
| Legacy web styling / components | **Tailwind CSS + shadcn/ui on `@base-ui/react` + CVA**, retained only for unmigrated surfaces | This is the currently shipped and tested implementation, so each route keeps it until its shared-system replacement passes behavior, accessibility, responsive, and visual parity. Wrapping the old implementation behind a new export does not count. P8 deletes the component tree, build integration, transitional token consumption, and dependencies; a permanent zero-legacy gate then prevents their return. The detailed primitive rationale below remains historical evidence for accessibility behavior the replacement must preserve. **Historical detail:** the enum control, Tooltip, Slider, Menu, Collapsible, and hand-built combobox carry keyboard, focus, description, and find-in-page behavior that migration tests must cover; Base UI replaced six separately versioned Radix packages, and its Tooltip requires an explicit description when the trigger label does not restate the content. |
...
| Component workshop + gallery | **Storybook 10** (`@storybook/react-vite`) + `@storybook/addon-a11y` (axe, in the workshop) + `@storybook/addon-themes` | The component gallery/contract and dev workshop (frontend-design §5); the official themes addon gives every story one persistent dark/light/system toolbar wired to the real `LoomarrProvider`, replacing per-story theme controls and making cross-theme review practical. It carries to the future mobile app via `@storybook/react-native` (Expo, on-device). Replaces the hand-rolled `/__gallery` registry. The CI gate (visual + a11y) is **one Playwright pass** over the offline `storybook-static` build. **Chromatic rejected** — hosted SaaS visual-diff, breaks the offline/self-hosted rule (§16) |
...
**Request-launched interactive operations belong to that generation too.** Filler acquisition,
compilation detection, and local-model pull return before their work finishes, but disconnecting the
HTTP request does not cancel accepted work. One application-owned launcher closes that apparent
contradiction: it detaches from the request, derives the operation context from the current generation,
rejects starts after quiesce begins, and tracks every accepted operation. Shutdown cancels and awaits
them, allowing only a bounded independent terminal-state write before it completes, so an old
generation cannot keep using its Store or event bus or overwrite recovery performed by its
replacement. Each operation also keeps a finite operation-specific timeout; model pull is bounded
rather than relying on Ollama to end the stream.

An **interactive operation** is this request-triggered, single-generation work, not a recurring or
distributed **Job** (§18.1). The launcher owns no durable queue, retry cadence, worker pool, or claim
protocol and therefore is not a second scheduler. Filler acquisition runs remain their existing
durable source of truth. Split detection and model pull persist small operation resources keyed by
the returned `jobId`, including progress and terminal proposal/outcome or a safe error. Their GETs
remain authoritative across dropped SSE and browser reconnects; startup marks a previous
generation's queued/running operation interrupted instead of resuming partial bytes. Request
contexts, raw `context.Background()` launches, and SSE-only terminal results are forbidden for these
operations because each loses one of disconnect independence, generation ownership, or recovery.
...
| `secretprotection` | Envelope encryption for database-backed secrets, with installation-key-wrapped data keys and safe rotation/replacement |
...
| `devbootstrap` | Harness-only provisioning of an isolated worktree identity through the real first-admin path (§11) |
...
- **Housekeeping (§5, §12, V32).** A **`housekeeping`** job (default daily) removes expired sessions, feed rows older than `activity.retention`, terminal notification intents and attempts past their fixed 30-day window, finished jobs past `JOBS_RETENTION`, denied proposals past `PROPOSALS_RETENTION`, and rolls discovery-quality receipts into their bounded aggregates (§17). **Proposals precede jobs**, so the purge never creates an orphaned `proposals.job_id`. All stages are attempted even if one fails; queued/sending notification work, other in-flight work, and the approval audit trail remain exempt.
