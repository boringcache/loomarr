...
func applyCacheWorkflowAuthorities(catalog *workflowAuthorityRegistry) {
	...
	}
	{
		...
		job.steps = map[string]workflowStepAuthority{}
		job.steps["git rev-parse HEAD\ncat .github/boringcache-upstream\ngit status --short\nlscpu\nsha256sum go.sum Cargo.lock rust-toolchain.toml web/pnpm-lock.yaml\n"] = cacheWorkflowStep(originalSteps["git rev-parse HEAD\ncat .github/boringcache-upstream\ngit status --short\nlscpu\nsha256sum go.sum Cargo.lock rust-toolchain.toml web/pnpm-lock.yaml\n"], 1, "Record source and runner identity", "bash", "", map[string]string{})
		job.steps["make fmt shellcheck privacy-verify vet tags-verify vet-tags lint agent-harness-test compose-verify release-verify go-race-verify"] = cacheWorkflowStep(originalSteps["make fmt shellcheck privacy-verify vet tags-verify vet-tags lint agent-harness-test compose-verify release-verify go-race-verify"], 5, "Static analysis and repository contracts", "", "", map[string]string{})
		job.steps["make go-shard-verify SHARDS=3"] = cacheWorkflowStep(originalSteps["make go-shard-verify SHARDS=3"], 6, "The test shards cover every package", "", "", map[string]string{})
		job.steps["make openapi-verify"] = cacheWorkflowStep(originalSteps["make openapi-verify"], 7, "OpenAPI spec is committed and current", "", "", map[string]string{})
		job.steps["make config-docs-verify"] = cacheWorkflowStep(originalSteps["make config-docs-verify"], 8, "Config docs are committed and current", "", "", map[string]string{})
		job.steps["make arch-docs-verify"] = cacheWorkflowStep(originalSteps["make arch-docs-verify"], 9, "The §2 package map is committed and current", "", "", map[string]string{})
		job.steps["make dev-docs-verify"] = cacheWorkflowStep(originalSteps["make dev-docs-verify"], 10, "Command reference is committed and current", "", "", map[string]string{})
		job.steps["make retired-verify"] = cacheWorkflowStep(originalSteps["make retired-verify"], 11, "Retired identifiers are gone from prose", "", "", map[string]string{})
		job.steps["make ci-lint"] = cacheWorkflowStep(originalSteps["make ci-lint"], 12, "Workflows are valid", "", "", map[string]string{})
		job.steps["make observability-verify"] = cacheWorkflowStep(originalSteps["make observability-verify"], 13, "Observability artifacts are provisionable", "", "", map[string]string{})
		workflow.jobs["run"] = job
		catalog.runs["ci-go-contracts.yml"] = workflow
		catalog.topology["ci-go-contracts.yml"] = workflowTopologyAuthority{jobs: map[string]int{"run": 14}}
		...
		catalog.actions[workflowActionKey{workflow: "ci-go-contracts.yml", job: "run", step: 4}] = "{\"name\":\"BoringCache go\",\"uses\":\"boringcache/one@404b744a2053da4cf963f13f615f7fafe94f3cf7\",\"with\":{\"cli-version\":\"${{ inputs.cli_version }}\",\"mode\":\"go\",\"trust-policy\":\"${{ (github.event_name == 'pull_request' || github.event_name == 'merge_group' || inputs.cache_access == 'restore') && 'restore' || 'auto' }}\",\"fail-on-cache-error\":true}}"
	}
	{
	...
	catalog.runs[replay].jobs["connect"] = workflowJobAuthority{condition: "github.repository == 'boringcache/loomarr' && (github.event_name == 'push' || inputs.phase == 'connect')", steps: map[string]workflowStepAuthority{
